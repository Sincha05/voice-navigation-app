...
//     const formData = new FormData();
//     formData.append("image", ocrFile); // must match backend param
...
function Dashboard({ user, setUser }) {
  ...
  const handleOCR = async (e) => {
    e.preventDefault();
    if (!capturedImage) return alert("⚠️ Capture an image first!");
    ...
    try {
      ...
      if (res.data.text) speakText(res.data.text);
      else alert("❌ No text detected in the image.");
    } catch (err) {
 ...
 const handleTTS = async (e) => {
    e.preventDefault();
    try {
      // Optional backend TTS call if needed for storage
      const res = await axios.post(`${backendURL}/tts/`, { text: ttsText });
      setTtsResult(res.data);
...
<div style={cardStyle}>
  ...
  <form onSubmit={handleOCR}>
    ...
    <button
      type="button"
      style={{ ...buttonStyle, background: "#ff69b4" }}
